...
class DataScreen extends StatelessWidget {
  const DataScreen({super.key, this.vendorSpeciality});

  final String? vendorSpeciality;

  Stream<List<Vendor>> readData() {
    final collection = FirebaseFirestore.instance.collection('vendor');

    return collection.snapshots().map((event) {
      final docs = event.docs;
      final List<Vendor> vendors =
          docs.map((doc) => Vendor.fromJson(doc.id, doc.data())).toList();

      if (vendorSpeciality == null) {
        return vendors
            .where((vendor) => vendor.profession == "Project Manager")
            .toList();
      } else {
        return vendors
